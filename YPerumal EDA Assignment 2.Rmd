...
This report aims to explore the two datasets "superbru-10k.Rds" and "fixtures.rds". These datasets contain information on predictions made users of the Superbru Super Rugby Fantasy league, and well as information on the fixtures that take place and their related results. 
...
To check how accurate a prediction is,the absolute difference between the predicted and actual results is taken. Additionally, an indicator is created to signal if a prediction was correct or not. Using a combination of these, points are allocated as per the Superbru scoring rules.  
...
Unzip the contents of the folder : "STA5092_EDA_Assigment2"
Copy the two data files(superbru-10k.Rds and fixtures.Rds) into the same folder
Open the file "STA5092_EDA_Assigment2.Rproj"
Use the file explore to open "Yperumal EDA Assignment 2.Rmd"(Double check the working directory is set to current folder where this file was opened)
...
b1 <-df %>% group_by(game_id)%>%
    ...
    geom_text_repel(aes(label=outlier),
                    ...
                    force = 3)+
    labs(title = ~ atop(paste('Histogram of'),
                              paste("Prediction Error Per Game")),
...
b2 <-df %>% group_by(game_id)%>%
    ...
    geom_text_repel(aes(label=outlier),
                    ...
                    min.segment.length = 0,
                    force = 3)+
    labs(title = ~ atop(paste('Histogram of Mean Squared'),
                              paste("Prediction Error Per Game")),
    subtitle = "Outlier Game ID displayed")+
    ylab("MSE")+
      theme(plot.title = element_text(size = 10,hjust = 0.5),
...
#If the mean guess was always better, then we would have more instances of the difference being positive.
# However, as we can see many fluctuations around 0, it doesn't look like this holds
# Plot the Difference
...
```
The graph above show the difference between the median prediction error and the mean prediction error. If the mean prediction(i.e. the wisdom of the crowds) is better than the average(given by the median), then the error for the median should always be larger, and thus the difference between the two values per game should have more values that are positive than not. However, we see many fluctuations around 0 as the games progress. 
...
```
If we look at the actual frequency of where the difference was positive and apply the logic above, it appears the wisdom of the crowd does not hold as there are many more instance where the average prediction outperformed the crowd's prediction.
...
```
The figure above plots the average error for fan predictions vs non fan predictions per game. It appears that fan prediction have slightly less variability in the errors and therefore may be slightly better than non fan predictions. We can confirm this by looking at the variance for each subset of predictions"

```{r}
# Variance for fan prediction matches
fan_var = var(fans[fans$fan_pred=="Fan",'mean_error'])
print(paste("Error Variance for games predicted by fans:",round(fan_var,2)))
# Variance for non fan predication matches
non_fan_var=var(fans[fans$fan_pred=="Non-Fan",'mean_error'])
print(paste("Error Variance for games not predicted by fans:",round(non_fan_var,2)))
#Looks like fans predict their team better
```
Games predicted by fans have a lower variance in their error, and therefore appear to be more dialed in to the accurate results,indicating fans are able to make better predictions for the teams they support.
