...
# 18 weeks in the season 
#rank per week for each user, first half of the season
# s1<-df %>%group_by(week,user_id)%>%
#     summarise(max_score = max(cumulative_score))%>%
#     mutate(rank = rank(-max_score,ties.method="first"))%>% #Minus score to rank desc order
#     arrange(week,rank)%>%
#     filter(week<=9)

# top 5 users with the highest average rank after 1st half of the season
# top_5s1 <-s1%>% group_by(user_id)%>%
#       summarise(mr=mean(rank))%>%
#       arrange(mr)%>%
#       head(5)
# top_5s1
#
# s2<-df %>% group_by(week,user_id)%>%
#     summarise(max_score = max(cumulative_score))%>%
#     mutate(rank = rank(-max_score,ties.method="first"))%>% #Minus score to rank desc order
#     arrange(week,rank)%>%
#     filter(week>9)
#
# top_5s2 <-df%>% group_by(user_id)%>%
#       summarise(mr=mean(rank))%>%
#       arrange(mr)%>%
#       head(5)
#
# top_5s2
#
# track_top5s2 <- df %>% filter(user_id %in% top_5s1$user_id)%>%
