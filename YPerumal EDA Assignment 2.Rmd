...
If I had to pick a top 20, scores dictate rankings first. Then we display based on smallest user_id value. These are users who signed up for Superbru earlier, and their loyalty has earned them the privilege of being displayed. In material terms it makes no difference to the actual points scored.
...
```
If we look at the actual frequency of where the difference was positive and apply the logic above, it appears the wisdom of the crowd does not hold as there are many more instance where the median prediction outperformed the mean crowd's prediction.
\newpage
...
fans%>% ggplot(aes(x=game_id,y=mean_error,colour=fan_pred))+
    geom_line()+
    labs(title = "Plot Displaying Mean Absolute Error per Game of Fan vs Non Fan Predictions")+
    xlab("Game ID")+
...
2. In the first 60 games we plot the cumulative points and cumulative error per user
3. To determine our experts, we calculate the "error per point" ratio and highlight users with the lowest 100 value. This means for each point earned, this user was as close as could be with their prediction to the actual result.
4. The top 100 user IDs are noted
5. A similar process followed in step 3 for the second 6 games of the season
...
s1 <- first_half%>%ggplot(aes(y=total_points,x=total_abs_error))+
              ...
              labs(title = "Scatterplot of Cumulative Points vs Cumulative Error per User, First 60 Games",
...
s2 <- second_half%>%ggplot(aes(y=total_points,x=total_abs_error))+
              ...
              labs(title = "Scatterplot of Cumulative Points vs Cumulative Error per User, Second 60 Games",
                   subtitle = "Users with 100 lowest error per point ratio highlighted")+
              xlab("Total Error")+
              ylab("Total Points")+
              scale_color_discrete(name = "Error Per Point Rank")
...
```{r Full Leaderboard, eval=FALSE, include=FALSE}
leaderboard<-df %>%group_by(week,user_id)%>%
    summarise(max_score = max(cumulative_score))%>%
    mutate(rank = rank(-max_score,ties.method="first"))%>% #Minus score to rank desc order
    arrange(week,rank)%>%
